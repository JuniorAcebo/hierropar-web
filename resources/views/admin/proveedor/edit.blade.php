...
<style>
    ...
    /* Estilo para el tipo de proveedor */
    .provider-type-badge {
    ...
    /* Iconos para tipos de proveedor */
    .provider-icon {
        margin-right: 0.5rem;
    }
</style>
...
<div class="container-fluid px-3 px-md-4">
    ...
    <div class="form-container">
        <form action="{{ route('proveedores.update',['proveedore'=>$proveedore]) }}" method="post">
            ...
            <!-- Informacion del tipo de proveedor -->
            <div class="info-header">
                ...
                <span class="provider-type-badge">
                    ...
                    <i class="fas fa-user provider-icon"></i>Persona Natural
                    @else
                    <i class="fas fa-building provider-icon"></i>Persona Juridica
                    @endif
                </span>
                </p>
            </div>
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select" name="documento_id" id="documento_id">
                        ...
                        <option value="{{$item->id}}"
                            {{ ($proveedore->persona->documento_id == $item->id || old('documento_id') == $item->id) ? 'selected' : '' }}>
                            {{$item->tipo_documento}}
                        </option>
                        @endforeach
                    </select>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Mostrar informacion del proveedor en consola para debugging
        console.log('Proveedor actual:', {
            tipo: '{{ $proveedore->persona->tipo_persona }}',
            razon_social: '{{ $proveedore->persona->razon_social }}',
            documento: '{{ $proveedore->persona->numero_documento }}'
        });
    });
</script>
@endpush
